...
async function runTests() {
  ...
  const mockDpRepository = {
    getColaboradoresByCompany: async (companyId: string) => {
      return [
        { id: "colab-1", nome: "Vinicius Trovo", salarioBase: 5000.00 },
        { id: "colab-2", nome: "Ana Silva", salarioBase: 2200.00 }
      ];
    }
  };
  ...
  try {
    ...
    const folhaResult = await folhaUseCase.execute({ companyId: "test-123", competencia: "2026-07" });
    
    console.log("✅ Folha de Pagamento calculada com sucesso!");
    folhaResult.forEach(f => {
      console.log(`   - Colaborador: \${f.nomeColaborador} | Líquido: R$ \${f.salarioLiquido.toFixed(2)} (INSS: R$ \${f.descontoInss.toFixed(2)})`);
    });
